        ...
        >
            ...
            >
                {columnKey => (<TableCell className='cursor-pointer' key={columnKey}>
                    {columnKey === 'action' ? (<DropDownDoted
                        onDelete={handleDelete}
                        params={params}
                        id={item.id}
                    />) : columnKey === 'icon' ? (
                        <i className={getKeyValue(item, columnKey)}/>) : columnKey === 'role' ? (roleMapping[getKeyValue(item, columnKey)] || 'Unknown') : columnKey === 'primary_valuta' ? (getKeyValue(item, columnKey) ? (
                        <i className='fas fa-star'/>) : ('')) : columnKey === 'publick' ? (<Checkbox
                        isSelected={getKeyValue(item, columnKey) === 1}
                        readOnly
                    />) : columnKey === 'types' && Array.isArray(getKeyValue(item, columnKey)) ? (getKeyValue(item, columnKey).join(', ')) : columnKey.includes('.') ? (columnKey.split('.').reduce((o, i) => (o ? o[i] : ''), item)) : columnKey === 'photo' ? (
                        <Image
